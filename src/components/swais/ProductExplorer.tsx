...
import { products, type Product, type ProductCode } from "@/data/products";

type Props = {
  variant?: "section" | "page";
};

const ProductDetail = ({ product, index }: { product: Product; index: number }) => (
  <motion.article
...
const ProductExplorer = ({ variant = "section" }: Props) => {
  ...
  // On the dedicated /products page we render every system in one continuous
  // scrollable view (no tabs).
  if (variant === "page") {
    return (
      <section id="systems" className="relative z-10 pt-12 md:pt-16 pb-24">
        <div className="container-x">
          <div className="space-y-10 md:space-y-14">
            {products.map((p, i) => (
              <div key={p.code} id={p.code.toLowerCase()} className="scroll-mt-32">
                <ProductDetail product={p} index={i} />
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="grid lg:grid-cols-12 gap-10 lg:gap-14">
          ...
          <div className="lg:col-span-8 min-w-0">
            <AnimatePresence mode="wait">
              ...
              >
                <ProductDetail product={product} index={0} />
              </motion.div>
            </AnimatePresence>
          </div>
        </div>
